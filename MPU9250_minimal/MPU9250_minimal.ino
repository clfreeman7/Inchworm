...
void setup()
{
  ...
  getAres();
  getGres();
  tt.every(1000/FREQUENCY,THEDAQRaw); //every (something seconds)
  xx=0;
...
void THEDAQRaw()
{
  ...
  dT = micros();
  I2Cread(MPU9250_ADDRESS, 0X3B, 14, &DAQ_Data[2]);  // Read the 14 raw data registers into data array
  I2Cread(MAG_ADDRESS, 0x09, 1, &c);  // Read the six raw data and ST2 registers sequentially into data array
  if(!(c & 0x08)) {
  I2Cread(MAG_ADDRESS, 0x03, 6, &DAQ_Data[16]);  // Read the six raw data and ST2 registers sequentially into data array
  }
  DAQ_Data[22] = (byte)(xx>>8); 
  DAQ_Data[23] = (byte)xx; //arduino time
  dT = micros()-dT;
  DAQ_Data[24] = (byte)(dT>>8); 
  DAQ_Data[25] = (byte)dT; //arduino time
